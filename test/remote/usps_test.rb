...
class USPSTest < Test::Unit::TestCase
  ...
  def test_tracking_with_bad_number
    ...
    assert_raises ResponseError do
      response = @carrier.find_tracking_info('EJ9580835',:test => true)
    end
    assert response.success?, response.message
    assert_instance_of Hash, response.params
    assert_instance_of String, response.xml
  ...
  def test_tracking_with_another_number
    response = nil
    assert_nothing_raised do
      response = @carrier.find_tracking_info('EJ958088694US', :test => true)
    end
    assert response.success?, response.message
    assert_instance_of Hash, response.params
    assert_instance_of String, response.xml
  ...
  def test_avs_response
     response = nil
     assert_nothing_raised do
       response = @carrier.verify_address( @locations[:usps_required_maryland], :test => true )
     end
     assert response.success?, response.message
     assert_instance_of Hash, response.params
     assert_instance_of String, response.xml
     puts response.locations[0].zip
  end
end